...
- **Keys 2 & 3 are hold-taps** (`mo_mkp`, tap-preferred, 200 ms): tap = mouse click (right/middle), hold = momentary gesture layer. Key 3 hold defaults to `DESKTOP`; the Key 2 + Key 3 combo toggles `RECTANGLE_OVERRIDE`, which changes Key 3 hold to `RECTANGLE`.
